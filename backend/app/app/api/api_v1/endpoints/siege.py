...
@router.get("/", response_model=List[Siege])
def get_all_salles(
    *,
...
@router.get("/search/", status_code=200, response_model=SiegeSearchResults)
def search_salles(
    *,
    keyword: Optional[str] = Query(None, min_length=3, example="siege"),
...
@router.get("/{salle_id}", status_code=200, response_model=Siege)
def fetch_salle(
    *,
    salle_id: int,
    db: Session = Depends(deps.get_db),
    ...
    if not result:
        ...
        # error otherwise.
        raise HTTPException(
            status_code=404, detail=f"Siege with ID {salle_id} not found"
        )
...
@router.post("/salle/", status_code=201, response_model=Siege)
def create_salle(
    *, salle_in: SiegeCreate, db: Session = Depends(deps.get_db)
) -> dict:
    """
...
@router.put("/{salle_id}", status_code=201, response_model=Siege)
def update_salle(
    *,
    # salle_in: SiegeUpdateRestricted,
    salle_in: SiegeUpdate,
    db: Session = Depends(deps.get_db),
    ...
    if not salle:
        raise HTTPException(
            status_code=400, detail=f"Siege with ID: {salle_in.id} not found."
        )
...
def delete_salle(
    *,
    ...
    if not salle:
        raise HTTPException(
            status_code=404, detail=f"Siege with ID: {id} not found."
        )
